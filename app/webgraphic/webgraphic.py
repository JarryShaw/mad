...
from webgraphic.group import group  # pylint: disable=E0611, E0401
...
class webgraphic:
            ...
            if re.match(self.ptr, s):
                ...
                if not ref:
                    ...
                    for k in range(len(self.tmp_mem)-1):
                        ...
                        if self.IsSimilar(domain_tmp, host) and not self.groups[self.tmp_index[index]].is_alone() and self.groups[self.tmp_index[index]].is_PC() == is_PC:  # noqa
                            # print("使用规则4连接")
                            # print(domain_tmp)
                            # print(host)
                    ...
                    if not fflag:
                        ...
                        for k in range(len(self.tmp_mem)-1):
                            ...
                            try:
                                ...
                                continue
                            index = len(self.tmp_mem) - 2 - k
                            if (self.IsSimilar(domain_tmp, ref_domain) or self.IsSimilar(self.groups[self.tmp_index[index]].ref_domain, ref_domain))and not self.groups[self.tmp_index[index]].is_alone() and self.groups[self.tmp_index[index]].is_PC() == is_PC:  # noqa
                                # print("使用规则2连接")
                                # print(index)
                                # print(s_tmp)
                                # print(s)
    ...
    def get_all_features_fortrainer(self, labled_url):
        # features=["content type","response lenth","number of referrals","time gap","url lenth","advertisement","presence of parents",lable]  # noqa
        features = []
        ...
        for x in self.groups:
            for i in range(len(x.id)):
                ...
                if url in labled_url:
                    lable = 1
                tmp = [content_type, response_lenth, number_of_ref, time_gap,
                       url_lenth, advertisement, presence_of_parents, lable]
                features.append(tmp)
        return features
    ...
    def getIndexById(self, id):
        ...
        middle = (low + high) / 2
        while middle != high:
            if self.groups[middle].id[0] < id:  # pylint: disable=E1126
                low = middle + 1
            elif self.groups[middle].id[0] > id:  # pylint: disable=E1126
                high = middle - 1
            else:
                index = middle
                break
    ...
    def IsSimilar(self, domain1, domain2):
        ...
        stop_words = ["www", "com", "cn", "bbs", "edu", "gov", "int", "mil",
                      "net", "org", "biz", "info", "pro", "name", "museum", "coop", "aero"]
